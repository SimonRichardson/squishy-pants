...
exports.eitherT = {
    'when testing eitherT ap with left should return correct value': _.check(
        function(a) {
            var monad = _.Left(a),
                transformer = _.Either.EitherT(monad),
                actual = transformer(_.Left(_.inc)).ap(transformer(monad));

            return _.expect(actual).toBe(transformer(_.Left(_.inc)));
        },
        [_.AnyVal]
    ),
    'when testing eitherT map with left should return correct value': _.check(
        function(a) {
            var eitherT = _.Either.EitherT(_.Left(a)),
                actual = eitherT(_.Left(a)).map(_.inc),
                expected = eitherT(_.Left(a));

            return _.expect(actual).toBe(expected);
        },
        [_.AnyVal]
    ),
    'when testing eitherT chain with left should return correct value': _.check(
        function(a, b) {
            var eitherT0 = _.Either.EitherT(_.Left(a)),
                actual = eitherT0(_.Left(a)).chain(
                    _.error('Failed if called')
                ),
                expected = eitherT0(_.Left(a));

            return _.expect(actual).toBe(expected);
        },
        [_.AnyVal]
    ),
    'when testing eitherT ap with right should return correct value': _.check(
        function(a) {
            var monad = _.Right(a),
                transformer = _.Either.EitherT(monad),
                actual = transformer(_.Right(_.inc)).ap(transformer(monad));

            return _.expect(actual).toBe(transformer(_.Right(a + 1)));
        },
        [Number]
    ),
    'when testing eitherT map with right should return correct value': _.check(
        function(a) {
            var eitherT = _.Either.EitherT(_.Right(a)),
                actual = eitherT(_.Right(a)).map(_.inc),
                expected = eitherT(_.Right(a + 1));

            return _.expect(actual).toBe(expected);
        },
        [_.AnyVal]
    ),
    'when testing eitherT chain with right should return correct value': _.check(
        function(a, b) {
            var eitherT0 = _.Either.EitherT(_.Right(a)),
                eitherT1 = _.Either.EitherT(_.Right(b)),
                actual = eitherT0(_.Right(a)).chain(
                    function() {
                        return eitherT1(_.Right(b));
                    }
                ),
                expected = eitherT0(_.Right(b));

            return _.expect(actual).toBe(expected);
        },
        [_.AnyVal, _.AnyVal]
    ),
    'when creating a eitherT and using chain should be correct value': _.check(
        function(a, b) {
            var monad = _.Either.of(1),
                transformer = _.Either.EitherT(monad);

            return _.expect(
                transformer(_.Either.of(b)).chain(function(x) {
                    return _.Either.EitherT(monad).of(x + 1);
                })
            ).toBe(_.Either.EitherT(monad).of(b + 1));
        },
        [Number, Number]
    ),
    'when creating a eitherT using eitherTOf and chain should be correct value': _.check(
        function(a, b) {
            return _.expect(
                a(_.Either.of(b)).chain(function(x) {
                    return _.Either.EitherT(_.Either.of(1)).of(x + 1);
                })
            ).toBe(_.Either.EitherT(_.Either.of(1)).of(b + 1));
        },
        [_.eitherTOf(Number), Number]
    ),
    'when creating a eitherT using eitherTOf and map should be correct value': _.check(
        function(a, b) {
            return _.expect(
                _.map(
                    a(_.Either.of(b)),
